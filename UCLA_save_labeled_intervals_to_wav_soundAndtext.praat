# save_labeled_intervals_to_wav_sound_files.txt
#
...
form Save intervals to small WAV sound files
	...
	sentence Folder /home/user/Desktop/suprasegs/noncomp/using-montreal-forced-aligner/fin_custom_data/utterances/
	comment Give an optional prefix for all SOUND filenames:
	sentence Prefix1 utterance_
	comment Give an optional prefix for all TEXTGRID filenames:
	sentence Prefix2 transcript_
...
for interval from start_from to end_at
	...
	if check = 0
		...
		filename$ = intname$
		intervalfile$ = "'folder$'" + "'prefix1$'" + "'suffix$'" + ".wav"
		indexnumber = 0
		while fileReadable (intervalfile$)
			indexnumber = indexnumber + 1
			intervalfile$ = "'folder$'" + "'prefix1$'" + "'suffix$''indexnumber'" + ".wav"
		endwhile
		...
		Extract part... intervalstart intervalend no
		filename$ = intname$
		intervalfile$ = "'folder$'" + "'prefix2$'" + "'suffix$'" + ".TextGrid"
		indexnumber = 0
		while fileReadable (intervalfile$)
			indexnumber = indexnumber + 1
			intervalfile$ = "'folder$'" + "'prefix2$'" + "'suffix$''indexnumber'" + ".TextGrid"
		endwhile
